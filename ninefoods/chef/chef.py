...
class Chef:
    ...
    def get_serving(self, food_name):
        for _category, _data in self.foods['Nine Foods'].items():
            for _food in _data:
                if _food['name'].lower() == food_name.lower():
                    _serving = _food['serving']
        return _serving

    def get_difficulty(self, food_name):
        for _category, _data in self.foods['Nine Foods'].items():
            for _food in _data:
                if _food['name'].lower() == food_name.lower():
                    _difficulty = _food['difficulty']
        return _difficulty

# murphy = Chef('chef/nine_foods.json')

# img = murphy.get_difficulty('Bread and Egg Toast')
# print(img)